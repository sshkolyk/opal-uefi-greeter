...
sudo mkdir -p mnt/"${GREETER_EFI_FILE%/*}"
sudo cp -r target/x86_64-unknown-uefi/release/opal-uefi-greeter.efi mnt/"$GREETER_EFI_FILE" || error
sudo cp -r config.ini mnt/config || error
